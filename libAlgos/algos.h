...
void printMinPath(const std::vector<std::vector<bool>>& maze, const std::vector<int>& path) {
    int r = maze.size();
    int c = maze[0].size();
    std::vector<std::vector<bool>> is_path(r, std::vector<bool> (c, false)); //матрица для ячеек, в которых есть маршрут
    for (auto& p : path) {
        int y = p / c;
        int x = p % r;
        is_path[x][y] = true;
    ...
    std::cout << "Кратчайший путь в лабиринте: \n";
    for (int i = 0; i < r; i++) {
        for (int j = 0; j < c; j++) {
            if (maze[i][j]) {
                std::cout << "##";
